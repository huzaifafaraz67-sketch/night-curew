...
const CHORES=[
  ...
  {k:'bolt',   t:'Turn the deadbolt on the front door'}
];
...
function interact(){
  ...
  switch(focus.userData.action){
    ...
    case 'bolt':
      ...
      if(S.bolted) markChore('bolt','The deadbolt goes over with a sound the whole floor can hear.');
      else { if(S.stage==='chores'){ S.chores.bolt=false; renderChores(); }
    ...
    case 'door':
      ...
      if(S.frontOpen&&S.bolted){ S.bolted=false;
        if(S.stage==='chores'){ S.chores.bolt=false; renderChores(); } }
